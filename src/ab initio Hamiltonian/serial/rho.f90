! Edited by Ishiaka Mansaray, to run with QE files generated by OCEAN runs
! cplex = 1 since we only use ABINIT for G.S calc.
!      program xpt
      subroutine rho(nx,nfft,inp,outp,loud)
      integer :: nx(3), nfft(3),i,j,k,ii,iz,iy,ix,dumint,i1,i2,i3
      integer :: inp,outp,ispden,nspden
      logical :: loud 
      character * 80 :: fstr
      integer, allocatable :: ilist(:,:)
      real, allocatable :: rhoofr(:,:,:),rhoofrtmp(:)
      ...
      allocate(rhoofrtmp(nfft(1)*nfft(2)*nfft(3)) )
...
!         rhoofr = reshape(rhoofrtmp,(/nfft(1),nfft(2),nfft(3)/))
!         write(6,*) rhoofrtmp
        enddo
       enddo
      enddo
