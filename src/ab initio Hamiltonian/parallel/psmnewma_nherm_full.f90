program planczos_full
        ...
        ! A full reorthogonalization is carried out in this program.
        !

        implicit none
        integer, parameter :: wp = kind(1.d0)
        
        ! smnewma2.f90 arrays and variables
        integer, parameter :: adat = 45, fdat = 47
        integer, parameter :: psitmp = 55
        integer, parameter :: eav = 80
        integer, parameter :: rcd = 46
        !
        integer, allocatable :: kk( :, : )
        real( kind = wp ), allocatable :: ebd(:,:), eblda(:,:)
        real( kind = wp ), allocatable :: xk(:,:)
        ...
        ! psi-->p, backpsi --> q, psi_p--> pp, backpsi_p --> qp, psi_m-->pm1, backpsi_m-->qm1
        ! tpsi--> tp, tbackpsi-->tq, term-->pterm, backterm-->qterm
        ! a--> zalpha, b-->zbeta, c-->zgamma
        ! n-->m, sys_order_m, niter-->mlanc, sys_order_n, iter-->k
        
        real( kind = wp ), allocatable :: allow(:), sfact(:),tmparray(:)
        complex( kind = wp ), allocatable :: pp(:), p(:), pm1(:), ptmp(:),ztmparray(:)
        complex( kind = wp ), allocatable :: qp(:), q(:), qm1(:), qtmp(:),&
                                                  tp(:),tq(:)
        
        ! Arrays, parameters for doing partial reorthogonalization
        complex( kind = wp ), allocatable :: Planc(:,:), Qlanc(:,:),Pj(:,:),Qj(:,:),&
                                            iden1(:,:),iden2(:,:)
        real( kind = wp ), allocatable :: fnorm(:)
        real( kind = wp ), parameter :: eps = 1*1.11E-16, eps1 = sqrt(eps)
        real( kind = wp ) :: frob,pterm,qterm
         
        integer :: m,n,mlanc
        real( kind = wp ) :: pnorm, vol, qnorm,inv_qlength, qinb(3),bvec(3,3)
  
        ! Lapack, BLAS functions
        integer :: vecdim
        real( kind = wp ) :: rterm1,rterm2
        complex( kind = wp ) :: zterm,zterm1,zterm2,lnorm1,lnorm2
        ...
        complex( kind = wp ), external :: hilbcd
        !
        complex( kind = wp ), external :: wvfdot
        !
        character(len=3) :: cmd, meth
        logical :: loud
        !
        integer :: nkret
        ...
        real( kind = wp ) :: f( 2 ), gres, gprc, ener, pi, fact, de, emax
        complex( kind = wp ) :: rm1
        complex( kind = wp ), allocatable :: v1( : ), v2( : ), cwrk( : )
        complex( kind = wp ), allocatable :: x( : ), xprev( : ), rhs( : ), pcdiv( : ),&         
                                                        bra( : ),xnew( : )
        ...
        ! ------ planc_nonherm_full arrays and variables -------
        ...
        integer :: k
        real(kind=wp) :: b0,temp1,tol,md, mc !,frob,fnorm(mlanc),rand_num(m)
        real(kind=wp) :: u0,DZNRM2,ZDOTC,lnorm,DLANGE ! fnorm, frobenius norm
!        real,parameter :: pi=3.141592654_wp
        integer,parameter :: sedsz=9
        integer           :: seed(sedsz),herm,mi,mj
        complex(kind=wp),parameter :: ii=cmplx(0.0_wp,1.0_wp)
        !  Non allocatable        
        complex(kind=wp),allocatable :: zalpha(:),zbeta(:),zdelta(:), &
                           zgamma(:),zborth(:),H(:,:) 
...
!        complex(kind=wp), allocatable :: Planc(:,:),Qlanc(:,:),Pj(:,:),Qj(:,:)
        ...
        ! ScaLAPACK variables
        integer, dimension(9) :: desc_Pj,desc_Qj,desc_Pj_loc,desc_Qj_loc,desc_iden_loc,desc_T_Qj_loc,&
                                 desc_pp_g,desc_qp_g,desc_pp_orth,desc_qp_orth,desc_pp_loc,desc_qp_loc,&
                                 desc_pp_orth_loc,desc_qp_orth_loc

        integer :: mloc_Pj,nloc_Pj,mloc_pp,nloc_pp,i_loc,j_loc,iG,jG
        complex(kind=wp),allocatable :: Pj_loc(:,:),Qj_loc(:,:),iden_loc(:,:),T_Qj_loc(:,:),&
        ...
        ! Set grid topology, aim for square grid
        !nprow=2; npcol=nproc/nprow
        nprow=int(sqrt(real(1.0_wp*nproc))); npcol=int(nproc/nprow)
!        nprow=int(nproc/2); npcol=2 
        ...
        ! Get system parameters from kandb.h (all procs fo this)
        biortho = .true.

        open( unit=99, file='kandb.h', form='formatted', status='unknown' )
        rewind 99
        call igetval( nkx, 'nkx  ' ); call igetval( nky, 'nky  ' ); call igetval( nkz, 'nkz  ' )
        nk = nkx * nky * nkz
        call igetval( ngx, 'ngx  ' ); call igetval( ngy, 'ngy  ' ); call igetval( ngz, 'ngz  ' )
        ng = ngx * ngy * ngz
        call igetval( nbv, 'nbv  ' ); call igetval( nbc, 'nbc  ' )
        call igetval( ilftl, 'ilftl' ); call igetval( ilfth, 'ilfth' )
        call igetval( irgtl, 'irgtl' ); call igetval( irgth, 'irgth' )
       ! nb = irgth - ilftl + 1
       ! if ( nb .ne. nbv + nbc ) stop 'metal option not yet reinstated'
        ...
        if ( (lflag .ne. 0) .and. ( (backf .ne. 0)  .or. (aldaf .ne. 0) ) ) stop 'ladder incomp w kxc'   	
        ...
        if (myrow .eq. 0 .and. mycol .eq. 0) then
          ...
          allocate( zalpha( 0 : niter ), zbeta( 0: niter + 1 ), zgamma( 0: niter + 1 ) )
          zalpha = 0.0_wp; zbeta = 0.0_wp; zgamma = 0.0_wp

          allocate( allow( n ), sfact( n ), p( n ), pp( n ), pm1( n ))
          allocate( q( n ), qp( n ), qm1( n ))
          allocate(tp(n),tq(n))
        !  allocate(H(n,n))
        !  H = 1.0_wp

        ! Allocate arrays for saving Lanczos vectors
        ! Planc for the p's, Qlanc for the q's

          if (nonherm .ne. 0) then
            allocate( Planc( n, niter ), Qlanc( n,niter ), fnorm(niter))
            Planc = 0.0
            Qlanc = 0.0
            fnorm = 0.0
          end if
          ...
          call getwgt_new( nk, nbv, nbc,ilftl,ilfth,irgtl,irgth,n, p ) ! for OCEAN data
          p = inv_qlength*p
          ...
          write(6,*) " "
          if ( backf .eq. 1 ) call bfnorm( p, ebd, nk, nb, nbc, nbv )
          !
...
!          q = 1.0_wp
          ...
          pnorm = dble( wvfdot( p, p, allow, n ) )
          pnorm = sqrt( pnorm )

          ! back psi
          qnorm = dble( wvfdot( q, q, allow, n ) )
          qnorm = sqrt( qnorm )

          !     if ( loud ) write ( 6, '(1a6,1x,1e15.8)' ) 'pnorm=', pnorm
          if ( loud ) write ( 6, * ) 'pnorm=', pnorm, 'backpnorm=', qnorm
          pterm = 1.0d0 / pnorm
          qterm = 1.0d0 / qnorm
        ...
        ! and carry out Lanczos procedure
                ...
                ! Make sure all procs have reached the same point in code
!                call blacs_barrier(icontxt, 'ALL')

                if (myrow .eq. 0 .and. mycol .eq. 0) then
                    allocate(Pj(m,k),Qj(m,k))
!                    allocate(Pj(m,1),Qj(m,1)) !test
                    Pj = 0.0_wp
                    Qj = 0.0_wp
                    Pj = Planc(:,1:k)
                    Qj = Qlanc(:,1:k)   
...
!                    call zcheck_orthog_frob(Pj,Qj,m,1,frob)  !test
                    fnorm(k) = frob
                    deallocate(Pj,Qj)
...
!                    tp=matmul(transpose(conjg(H)),p) ! doing A^H*w_j
                    ! Act on psi and backpsi, getting tpsi, tbackpsi, according to 
                    ! lanczos_non_herm routine in lanczos_8.f90
...
!                    ! p
                    ...
                    call act_cmplx_dag( tp, u, ebd, kk, nk, nkx, nky, nkz, ng, &
                                        ngx, ngy, ngz, nb, nbc, nbv, xk, nkret, kret,&
                                     mr, lflag, bflag, backf, aldaf, allow, sfact, bande )
!                    call act( tp, u, ebd, kk, nk, nkx, nky, nkz, ng, &
!                                        ngx, ngy, ngz, nb, nbc, nbv, xk, nkret, kret,&
!                                     mr, lflag, bflag, backf, aldaf, allow, sfact, bande )
                    ...
                    call act_cmplx( tq, u, ebd, kk, nk, nkx, nky, nkz, ng, &
                                    ngx, ngy, ngz, nb, nbc, nbv, xk, nkret, kret, &
                                    mr, lflag, bflag, backf, aldaf, allow, sfact, bande )
!                    call act( tq, u, ebd, kk, nk, nkx, nky, nkz, ng, &
!                                    ngx, ngy, ngz, nb, nbc, nbv, xk, nkret, kret, &
!                                    mr, lflag, bflag, backf, aldaf, allow, sfact, bande )
                                    
!                    zterm=dot_product(p,tq) ! Following the SLEPc paper for Hermitian case
                    zterm=wvfdot(p,tq,allow,m) ! need allow to match serial code
                    write(6,*) ""
                    write(6,*) "====== Before Lanczos, overlap ========"
                    write(6,*) " <p|A|q> =", zterm
                    write(6,*) ""
                    
                    !   Including this Paige-like regularization, from hermitian lanczos,
                    ! for better convergence
                    qp=tq-zgamma(k)*qm1 ! v^_(j+1)
                    pp=tp-conjg(zbeta(k))*pm1 ! v^_(j+1)
                    
                    ! calculating alpha
!                    zterm=dot_product(p,qp) ! Following the SLEPc paper for Hermitian case
                    zterm=wvfdot(p,qp,allow,m) ! need allow to match serial code
                    zalpha(k) = zterm
                ...
                if (biortho) then
                        if(myrow .eq. 0 .and. mycol .eq. 0)then
                          print *
                          print *, "Starting rebiorthogonalization"
                          print *
...
!                          print *
                        ...
                        !              icontxt,lld,info)
                        call descinit(desc_Pj_loc,sys_order_m,sys_order_n,block_size_m,block_size_n,0,0,&
                                      icontxt,lld,info)
                        call descinit(desc_Qj_loc,sys_order_m,sys_order_n,block_size_m,block_size_n,0,0,&
                                      icontxt,lld,info)
                        ! Temp matrix(m,m)
                        call descinit(desc_iden_loc,sys_order_m,sys_order_m,block_size_m,block_size_m,0,0,&
                                      icontxt,lld,info)
                        call descinit(desc_T_Qj_loc,sys_order_m,sys_order_m,block_size_m,block_size_m,0,0,&
                                      icontxt,lld,info)

                        ! 1-D
                        call descinit(desc_pp_loc,sys_order_m,1,block_size_m,1,0,0,&
                                      icontxt,lld,info) 
                        call descinit(desc_qp_loc,sys_order_m,1,block_size_m,1,0,0,&
                                      icontxt,lld,info) 

                        ! orthogonalized vectors
                        call descinit(desc_pp_orth_loc,sys_order_m,1,block_size_m,1,0,0,&
                                      icontxt,lld,info) 
                        call descinit(desc_qp_orth_loc,sys_order_m,1,block_size_m,1,0,0,&
                                      icontxt,lld,info) 

                       ! Distribute the matrix Pj and Qj and have each processor print theirs locally.
!                        print *
!                        print *, "Starting distribution of global arrays"
!                        print *
                       ! 2-D arrays
...
!                        call blacs_barrier(icontxt, 'ALL')
                        ...
                        call pzgeadd('n',sys_order_m,sys_order_n,alpha,Pj,iA,jA,desc_Pj,beta,&
                                     Pj_loc,iA,jA,desc_Pj_loc)
                        call pzgeadd('n',sys_order_m,sys_order_n,alpha,Qj,iA,jA,desc_Qj,beta,&
                                     Qj_loc,iA,jA,desc_Qj_loc)
...
!                        print *
!                        print *, "Finished  distribution of global arrays"
!                        print *

                       ! Initialize the parts of identity matrix owned by processors
                       ! Use the global indices to set iden(i,i) = 1.0_wp
                        iden_loc = 0.0_wp
                        do i_loc=1,mloc_A
                           do j_loc=1,mloc_A
                              i = indxl2g(i_loc,block_size_m,myrow,0,nprow)
                              j = indxl2g(j_loc,block_size_m,mycol,0,npcol)
                              if (i .eq. j) then
                                iden_loc(i_loc,j_loc) = 1.0_wp
                              endif
                           enddo
                        enddo
             ...
             !           desc_A,desc_B,desc_C,desc_x,desc_y)
                        ...
                        call mmdag_v(Pj_loc,Qj_loc,T_Qj_loc,pp_loc,pp_orth_loc,alpha,beta,m,k,&
                                  desc_Pj_loc,desc_Qj_loc,desc_T_Qj_loc,desc_pp_loc,desc_pp_orth_loc)

!                        call do_orthog(Pj_loc,Qj_loc,T_Qj_loc,iden_loc,pp_loc,pp_orth_loc,alpha,beta,m,k,&
!                                  desc_Pj_loc,desc_Qj_loc,desc_T_Qj_loc,desc_iden_loc,desc_pp_loc,desc_pp_orth_loc)
!
                     ! For qp_orth Qj*Pj^H
                        T_Qj_loc = 0.0_wp
                        call mmdag_v(Qj_loc,Pj_loc,T_Qj_loc,qp_loc,qp_orth_loc,alpha,beta,m,k,&
                                  desc_Qj_loc,desc_Pj_loc,desc_T_Qj_loc,desc_qp_loc,desc_qp_orth_loc)

!                        call do_orthog(Qj_loc,Pj_loc,T_Qj_loc,iden_loc,qp_loc,qp_orth_loc,alpha,beta,m,k,&
!                                  desc_Qj_loc,desc_Pj_loc,desc_T_Qj_loc,desc_iden_loc,desc_qp_loc,desc_qp_orth_loc)
...
!                        print *
!                        print *, "Finished  distribution of global arrays and mmv prod"
!                        print *
                        ! Deallocate space
                        if(myrow .eq. 0 .and. mycol .eq. 0)then
...
!                          qp = qp_orth(:,1)
                       ...
                       ! deallocate(A)
                        deallocate(Pj_loc,Qj_loc,T_Qj_loc,iden_loc,pp_loc,qp_loc,&
                ...
                if(myrow .eq. 0 .and. mycol .eq. 0) then
!                    print *
!                    print *," Updating the coefficients"
!                    print *
                    zterm1 = wvfdot(qp,pp,allow,n) ! need allow to match serial code

        !  A measure of the biorthogonality
!                    zborth(k) = zterm1
                    lnorm1 = sqrt(abs(zterm1))  ! following yambo paper
                    lnorm2 = conjg(zterm1)/lnorm1
!                    print *
!                    print *,"lnorm1 =",lnorm1,"lnorm2 =",lnorm2
!                    print *
...
!        ! Check level of orthogonality right after adding new vecs              
!                      call zcheck_orthog_frob(Planc(:,1:k+1),Qlanc(:,1:k+1),m,k+1,frob)
!                      print *
!                      print *," Right after re orthog, level of orthog =",frob
!                      print *
!                      zterm = wvfdot(p,Qlanc(:,k+1),allow,m)
!                      zterm1 = wvfdot(q,Planc(:,k+1),allow,m)
!                      print *,"Overlap of new vectors"
!                      print *,"<p|q> =",zterm
!                      print *,"1 - <p|q> =",1.0_wp - zterm
!                      print *
!                      print *,"<q|p> =",zterm1
!                      print *,"1 - <q|p> =",1.0_wp - zterm1
!                      print *
                    endif
                endif ! ----- Only processor (0,0)  Does the Lanczos operations
...
!!  Saving alpha, beta to array
        if(myrow .eq. 0 .and. mycol .eq. 0) then
           mlanc = niter
           open(10,file='par_lanc_nh_mat_full_64_procs.dat')
           open(11,file='par_fnorm_nh_mat_full_64_procs.dat')
           rewind(10)
           rewind(11)
           ...
           do i=1,mlanc
             ...
             write(11,'(1i3,1e16.8)') i,fnorm(i) !,abs(aimag(zborth(i)))
             write(6,*) ""
           ...
           call cadump( adat, 'a.dat', pnorm, nk, k, zalpha )
           call cadump( adat, 'b.dat', pnorm, nk, k + 1, zbeta )
           call cadump( adat, 'c.dat', pnorm, nk, k + 1, zgamma )
           ...
           deallocate( ebd,u,kk,xk,zalpha,zbeta,zgamma,allow,pp,p,pm1,tp,qp,q,qm1,tq )
           if (nonherm .ne. 0) deallocate( Planc, Qlanc, fnorm )
...
!                     print*, 'PE:',myrow,mycol,' y=',y
        ...
        subroutine do_orthog(A,B,C,D,x,y,alpha,beta,sys_order_m,sys_order_n, &
                        desc_A,desc_B,desc_C,desc_D,desc_x,desc_y)

                     ! Does the GS orthog of a vector in parallel. It follows the most 
                     ! rigorous way of doing this, using matrix operations all the way
                     ! Uses the hermitian conjugate for matrix B instead.
                     ! A(m,k), B(n,k) --> C(m,n) 
                     ! I = (m,m) --> p_orthog = (I - P*Q^dag)*p
                     ! p_orthog = matmul((I-P*Q^dag),p)
                     ! Uses pzgemm, pzgeadd, pzgemv
...
!                     real(kind=wp) :: alpha,beta
                     ...
                     complex(kind=wp), dimension(:,:), intent(in) :: B
                     complex(kind=wp), dimension(:,:), intent(inout) :: C
                     complex(kind=wp), dimension(:,:), intent(inout) :: D 
                     complex(kind=wp), dimension(:,:), intent(in) :: x
                     complex(kind=wp), dimension(:,:), intent(out) :: y
                     complex(kind=wp) :: alpha,beta

                     integer, intent(in) :: sys_order_m,sys_order_n
                     integer, dimension(9), intent(in) :: desc_A,desc_B,desc_C,desc_D,desc_x,desc_y
                     integer :: iA,jA,iB,jB,iC,jC,ix,jx,nrhs
                     integer :: info
                     integer, dimension(sys_order_m+block_size_m) :: ipvt
...
!                     print*, 'PE:',myrow,mycol,' C=',C
                     ...
                     ! Now use pzgeadd to subtract P*Q^dag from the identity matrix
                     ! I = (beta*I - alpha*P*Q^dag), --> beta = 1.0_wp, alpha = -1.0_wp
                     ! call pzgeadd('n',sys_order_m,sys_order_n,alpha,A_g,iA,jA,desc_A_g,beta,A,iA,jA,desc_A)
                     alpha = -1.0_wp
                     beta = 1.0_wp
                     call pzgeadd('n',sys_order_m,sys_order_m,alpha,C,iA,jA,desc_C,beta,&
                                     D,iA,jA,desc_D)
...
!                                 beta,y,ix,jx,desc_y,1)
                     ...
                     alpha = 1.0_wp
                     beta = 0.0_wp
                     call pzgemv('n',sys_order_m,sys_order_m,alpha,D,iC,jC,desc_D,x,ix,jx,desc_x,1,&
                                 beta,y,ix,jx,desc_y,1)

!                     print *
!                     print *,"Completed do_orthog"
!                     print *
!                     if (info .ne. 0)print*, 'pzgemv info: ',info

!                     print*, 'PE:',myrow,mycol,' y=',y
        end subroutine do_orthog
