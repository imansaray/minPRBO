program planczos_pro
        ...
        ! ------ planc_nonherm_pro arrays and variables -------
        ...
        real(kind=wp) :: b0,temp1,tol,term_re,term_im,md,&
                         mc
        real(kind=wp) :: u0,DZNRM2,ZDOTC,lnorm,DLANGE ! fnorm, frobenius norm
        integer,parameter :: sedsz=9
        integer           :: seed(sedsz),herm,mi,mj,orthtot,&
                             poterm,qoterm,oterm,orthfreq
        ...
        integer, allocatable :: porthopt(:),qorthopt(:),porthindx(:),&
                                qorthindx(:)
        complex(kind=wp), allocatable :: pw_jk(:),qw_jk(:)
        logical :: biortho
...
!        nprow=int(nproc/2); npcol=2 
        ...
        ! Get system parameters from kandb.h (all procs do this)
        biortho = .true.
...
!                    zterm=dot_product(p,qp) ! Following the SLEPc paper for Hermitian case
                ...
                !!  Calculate local orthog, change vp-->tp, vq-->tq
                            tp = 0.0
                            tq = 0.0
!                            zterm1 = dot_product(qp,pp) ! following netlib Alg 7.13
                            zterm1 = wvfdot(qp,pp,allow,m) ! need allow to match serial code
                            lnorm1 = sqrt(abs(zterm1))  ! following yambo paper
                            lnorm2 = conjg(zterm1)/lnorm1
                            tp=(1.0/conjg(lnorm2))*pp ! w_(j+1)=w^_(j+1)/beta_(j+1)
                            tq=(1.0/lnorm1)*qp! v_(j+1)=v^_(j+1)/delta_(j+1)
                            pw_jk = matmul(conjg(transpose(Qj)),tp)
                            qw_jk = matmul(conjg(transpose(Pj)),tq)
        ...
        !                          print *
                               ...
                               ! 2-D arrays
                               ! call   
                               !pzgeadd('n',sys_order_m,sys_order_n,alpha,A_g,iA,jA,desc_A_g,beta,A,iA,jA,desc_A)
...
!                    zterm1 = dot_product(qp,pp) ! following netlib Alg 7.13
        ...
        ! Update orthgs info
                    write(orth,*) k,oterm,2*oterm,orthfreq,orthtot        
