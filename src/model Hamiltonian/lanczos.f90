...
! and partial reorthogonalization strategies.
! Subsequently, the calculation of the resolvent (w-H)^-1, in terms of
...
!                    
        ...
        complex(kind=8) ::zalpha(mlanc),zbeta(mlanc+1),zdelta(mlanc+1), &
                &           zgamma(mlanc),zborth(mlanc),Planc(m,mlanc)
        ...
        real(kind=8), allocatable ::Htest(:,:),fnorm(:),estnorm(:),&
                                    HGS(:,:)
        ...
        ! parameters for initializing elements of the matrix,  for i and j respectively
        mi=6
        mj=5
        md = 1.0d0
        mc = 0.0d0
...
!          v0(i) = rand_num(i)
        ...
        ! File to saving the Lanczos coefficients, loss of orthogonality, dielectric function to file
         ...
         ! hermitian
!        open(10,file='lanc_coeffs_herm_no_m_1000_mlanc_200.dat')
!        open(14,file='fnorm_herm_no_m_1000_mlanc_200.dat')
!        open(11,file='resolvent_exact_herm_m_1000.dat')
!        open(111,file='resolvent_lanc_herm_no_m_1000_mlanc_200.dat')

         ! non-symm, real
!        open(10,file='lanc_coeffs_non_symm_no_m_1000_mlanc_200.dat')
!        open(14,file='fnorm_non_symm_no_m_1000_mlanc_200.dat')
!        open(11,file='resolvent_exact_non_symm_m_1000.dat')
!        open(111,file='resolvent_lanc_non_symm_no_m_1000_mlanc_200.dat')

        ! non hermitian
!        open(10,file='lanc_coeffs_non_herm_no_m_1000_mlanc_300_spec_range_22.dat')
!        open(14,file='fnorm_non_herm_no_m_1000_mlanc_300_spec_range_22.dat')
!        open(11,file='resolvent_exact_non_herm_m_1000_spec_range_22.dat')
!        open(111,file='resolvent_lanc_non_herm_no_m_1000_mlanc_300_spec_range_22.dat')

        open(10,file='lanc_coeffs_non_herm_minprbo_m_1000_mlanc_300_spec_range_22.dat')
        open(14,file='fnorm_non_herm_minprbo_m_1000_mlanc_300_spec_range_22.dat')
        open(11,file='resolvent_exact_non_herm_m_1000_spec_range_22.dat')
        open(111,file='resolvent_lanc_non_herm_minprbo_m_1000_mlanc_300_spec_range_22.dat')

!        open(10,file='lanc_coeffs_non_herm_full_m_1000_mlanc_300_spec_range_22.dat')
!        open(14,file='fnorm_non_herm_full_m_1000_mlanc_300_spec_range_22.dat')
!        open(11,file='resolvent_exact_non_herm_m_1000_spec_range_22.dat')
!        open(111,file='resolvent_lanc_non_herm_full_m_1000_mlanc_300_spec_range_22.dat')
        ...
        if (herm==1) then
           ...
           print *
             ...
             do j=1,m
                do i=1,j
                   hterm=del_spect*(mod(i,mi)+ii*(mod(j,mj)))
                   H(i,j)=hterm
                   H(j,i)=conjg(hterm)
                enddo
             enddo
         ...
         ! Based on example from : "The Lanczos Alg with partial reorthogonalization"  H(i,i) = 1.0d0*i**2
               H(i,i) = del_spect*i**1
               if (i .gt. mhalf) then
                  H(i,i) = sgn_star*del_spect*mod(i,mhalf)
               endif
               Iden(i,i) = 1.0d0
          enddo
           H(m,m) = sgn_star*del_spect*mhalf
...
!              Hmat(i,i)= md*mod(i,mi)
          ...
          ! Based on example from : "The Lanczos Alg with partial reorthogonalization"  H(i,i) = 1.0d0*i**2
               ...
               if (i .gt. mhalf) then
                  Hmat(i,i) = sgn_star*del_spect*mod(i,mhalf)
               endif
               Iden(i,i) = 1.0d0
          enddo
           Hmat(m,m) = sgn_star*del_spect*mhalf
...
!                   hterm=del_spect*(mod(i,mi)+ii*(mod(j,mj)))
!                   H(i,j)=hterm
!                   H(j,i)=del_spect+conjg(hterm)
!                enddo
!             enddo

           ! diagonal
           do i=1,m
!               hterm=del_spect*(mod(i,mi)+ii*(mod(i,mi)))
               hterm=del_spect*(i+0.0d0*ii*i*abs(sin(1.0d0*i)))
               H(i,i) = hterm
               Iden(i,i) = 1.0d0
          enddo
...
!! Dielectric function: e(w) = 1.0 - <v_0|(w*I + i*eta - H)^-1|v_0>
   ...
   !  Calculating norm of the initial vector , v_0
      ...
      do i=0,omega_max
        ...
        print *
        print *, "omega = ",omega
        print *,"<v_0|(w*I + i*eta - H)^-1|u_0> = ",zterm
!        print *
!        print *,"<v_0|(-w*I + i*eta - H)^-1|u_0> = ",zterm1
!        print *
...
! Carries out Lanczos algorithm for a real nonsymmetric matrix.
        ...
        real(kind=8) ::alpha(mlanc),beta(mlanc),delta(mlanc), &
...
! re biorthogonalization strategy (minPRBO).
...
!  Print out final total orthogonalizations
        ...
        write(6,*) " "
        return
        end subroutine lanc_non_herm_minprbo
